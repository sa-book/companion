...
Here you can dowload the tweaked version of `seqplot.rf`: `r icon::fa("download")` [relfreqseq-obj.R]("source/relfreqseq-obj.R").
...
The quality and the appearance of relative frequency sequence plots heavily depend on the underlying analytical choices. Below we illustrate how the plot looks like with a different sorting criterion, i.e. the time spent in marriage. The marital states are the last threes states of the alphabet (`alphabet(partner.child.year.seq)[7:9]`), the respective state frequencies are extracted by TraMineR's `seqistatd` function, and summed up using `rowSums`. The plot is generated in excactly the same way as above. We only have to replace `k37.mds` with `k37.mardur`.
...
The dissimilarities to medoid plot and the lower R2 show that this sorting criterion performs worse than the previous one. In the next figure we re-plot only the two medoid sequences of the two solution to ease the comparison. In terms of code this means, that the box-whisker-plot from the previous code snippet (`boxplot(k37.mds[[3]]~k37.mds[[2]] ...`) has to replaced by another `seqIplot`. In addition, we slightly adjust the two index plots' titles to print the repsective R2s.
