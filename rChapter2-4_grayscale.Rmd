...
In the book we elaborate on the pros and cons of sequence index plots. Based on this discussion we come to the conclusion that standard index plots of all sequences in large data sets rarely make sense. This is even more true if colors are not available for visualization.
Accordingly, the subsequent example only renders a selection of 10 representative sequences extracted by `seqrep` using sequences with a yearly granularity.
