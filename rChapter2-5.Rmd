...
## Bonus material: <br/>Generalized version of the sequence quality index
...
$$
The vector $q$ is defined to indicate the quality of a state. The quality vector assigns values between 0 and 1 to every state of the alphabet. If not specified differently, the state at the bottom of the quality hierarchy is coded 0, whereas the state at the top is coded 1. For the states in-between each step up the hierarchy increases the value on the vector by $1/(l(A) - 1)$, with $l(A)$ indicating the length of the alphabet. 
...
bonus <- as_tibble(ex1.seq) %>% 
  mutate_all(as.numeric) %>%
  mutate_all(~(case_when(. == 1 ~ 0,
                         . == 2 ~ 1/3,
                         . == 3 ~ 2/3,
                         . == 4 ~ 1,)))

bonus <-  map_dfc(1:ncol(bonus), ~bonus[,.x]*.x) %>%
  as_tibble() %>%
  transmute(Generalized = rowSums(.)/15) %>%
  mutate(Original = seqsuccess(ex1.seq, success = 4),
         Sequence = print(ex1.seq, format = "SPS")) %>%
  select(Sequence, Original, Generalized)

```

```{r, echo=FALSE}
kable(bonus, digits = 2) %>%
